function imdb = imdb_from_plane(root_dir, image_set, year, flip)
...
cache_file = ['./imdb/cache/imdb_plane_' year '_' image_set];
if flip
...
%   VOCopts.testset = image_set;
  addpath(root_dir);
  imdb.name = ['plane_' year '_' image_set];
  imdb.image_dir = root_dir%fileparts(VOCopts.imgpath);
  txt_name=sprintf('plane_%s.txt',image_set);
  imdb.image_ids = textread((txt_name), '%s');
  imdb.extension = 'bmp';
  imdb.flip = flip;
  if flip
...
%   imdb.details.VOCopts = VOCopts;
  ...
  imdb.eval_func = @imdb_eval_plane;
  imdb.roidb_func = @roidb_from_plane;
  imdb.image_at = @(i) ...

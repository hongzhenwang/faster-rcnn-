function res = imdb_eval_plane(cls, boxes, imdb, cache_name, suffix)
...
VOCopts.testset='/home/whz/DNN/faster_rcnn-master/plane_test.txt';
VOCopts.minoverlap=0.5;
VOCopts.annopath='/home/whz/image_set/plane20151105/AnnotationMat/%s.mat';
image_ids = imdb.image_ids;
test_set = 'test';%VOCopts.testset;
year = '2015';%VOCopts.dataset(4:end);
...
res_fn = sprintf([conf.cache_dir,'/%s_%s.txt'], res_id, cls);
VOCopts.detrespath=[conf.cache_dir,'/%s_%s.txt'];
...
do_eval = (str2num(year) == 2015) |(str2num(year) <= 2007) | ~strcmp(test_set, 'test');
if do_eval
  % Bug in VOCevaldet requires that tic has been called first
  tic;
  [recall, prec, ap] = VOCevaldet_plane(VOCopts, res_id, cls, draw_curve);
  ap_auc = xVOCap_plane(recall, prec);

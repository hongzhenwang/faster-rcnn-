function roidb = roidb_from_plane(imdb, varargin)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
plane.annopath='/home/whz/image_set/plane20151105/AnnotationMat/%s.mat';
  ...
  if ~imdb.flip
      for i = 1:length(imdb.image_ids)
        ...
        try
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          load(sprintf(plane.annopath, imdb.image_ids{i}));
          voc_rec = rec;
        catch
          voc_rec = [];
        end
      ...
      for i = 1:length(imdb.image_ids)/2
        ...
        try
          load(sprintf(plane.annopath, imdb.image_ids{i*2-1}));
          voc_rec = rec;
        catch
          voc_rec = [];
        end
        if ~isempty(regions)
            ...
            [~, image_name2] = fileparts(regions.images{i});
            if imdb.flip_from(i*2) == i*2-1 && strcmp(image_name1, image_name2)
                i;
            else
                i;
%                 assert(strcmp(image_name1, image_name2));
...
if ~isempty(boxes)
     error('roidb_from_plane: inited boxes is not empty');
    boxes = boxes(:, [2 1 4 3]);
    if flip
...
if isfield(voc_rec, 'objects')
      ...
      tem_whz={voc_rec.objects(valid_objects).class};
      tem2_whz=cat(1,tem_whz{:});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%add gt_roi%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
gt_ori= cat(1, voc_rec.objects(valid_objects).ori);
rec.ori = uint8(cat(1, gt_ori, zeros(num_boxes, 1)));
if flip
    ori_transform=[5 4 3 2 1 8 7 6];
    ori_new=ori_transform(gt_ori);
    rec.ori = uint8(cat(1, gt_ori, zeros(num_boxes, 1)));
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

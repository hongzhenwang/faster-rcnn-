function script_faster_rcnn_plane_lenet_4a_bn()
...
opts.do_val                 = true; 
% model
model                       = Model.lenet_4a_for_Faster_RCNN_plane_bn;
% cache base
cache_base_proposal         = 'plane_lenet_bn';
...
dataset                     = Dataset.plane_trainval(dataset, 'train', use_flipped);
dataset                     = Dataset.plane_test(dataset, 'test', false);
...
try load('anchors_plane.mat');conf_proposal.anchors=anchors_plane.anchors; conf_proposal.output_width_map=anchors_plane.output_width_map; conf_proposal.output_height_map=anchors_plane.output_height_map;
catch
[conf_proposal.anchors, conf_proposal.output_width_map, conf_proposal.output_height_map] ...
                            = proposal_prepare_anchors(conf_proposal, model.stage1_rpn.cache_name, model.stage1_rpn.test_net_def_file);
anchors_plane.anchors=conf_proposal.anchors;
anchors_plane.output_width_map=conf_proposal.output_width_map;
anchors_plane.output_height_map=conf_proposal.output_height_map;
...
% test
%原来的roidb——train/test中只有gt boxes，现在加入stage1中生成的bboxes
try load('roi_new_plane.mat');dataset.roidb_train= roi_new_plane.train;dataset.roidb_test= roi_new_plane.test;
catch
    ...
    roi_new_plane.train=dataset.roidb_train ;
    roi_new_plane.test=dataset.roidb_test;
...
fprintf('\n***************\nstage ori\n***************\n');
% train
model.stage_ori.init_net_file = model.stage1_fast_rcnn.output_model_file;
...
dataset.roidb_test       	= Faster_RCNN_Train.do_proposal_test(conf_proposal, model.stage2_rpn, dataset.imdb_test, dataset.roidb_test);
opts.final_mAP              = Faster_RCNN_Train.do_fast_rcnn_test_ori(conf_fast_rcnn, model.stage_ori, dataset.imdb_test, dataset.roidb_test);
Faster_RCNN_Train.gather_rpn_fast_rcnn_models_with_ori(conf_proposal, conf_fast_rcnn, model, dataset);
% % save final models, for outside tester
% Faster_RCNN_Train.gather_rpn_fast_rcnn_models(conf_proposal, conf_fast_rcnn, model, dataset);model.stage2_rpn.nms        = model.final_test.nms;
% dataset.roidb_test       	= Faster_RCNN_Train.do_proposal_test(conf_proposal, model.stage2_rpn, dataset.imdb_test, dataset.roidb_test);

function [im_blob, ori_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_blob] = ori_get_minibatch(conf, image_roidb)
...
% --------------------------------------------------------
    ...
    ori_blob = single(permute(ori_blob, [3, 4, 2, 1]));
...
function [labels,ori_labels, overlaps, rois, bbox_targets, bbox_loss_weights] = ...
...
%     overlaps = image_roidb(1).max_overlaps;
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     [overlaps_ori, labels_ori] = max(image_roidb(1).ori_overlap, [], 2);
    % Select sampled values from various arrays

function save_model_path = ori_train_with_bboxes(conf, imdb_train, roidb_train, varargin)
...
%% inputs
    ...
    ip.addParamValue('val_iters',       30,            @isscalar); 
...
%% try to find trained model
    ...
    cache_dir = fullfile(pwd, 'output', 'ori_cachedir', opts.cache_name, imdbs_name);
...
%% making tran/val data
    ...
    [image_roidb_train, bbox_means, bbox_stds]...
                            = ori_prepare_image_roidb(conf, opts.imdb_train, opts.roidb_train);
    fprintf('Done.\n');
    
    if opts.do_val
        fprintf('Preparing validation data...');
        [image_roidb_val]...
                                = ori_prepare_image_roidb(conf, opts.imdb_val, opts.roidb_val, bbox_means, bbox_stds);
        fprintf('Done.\n');
...
%% training
    ...
    while (iter_ < max_iter)
        ...
        [im_blob, ori_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_weights_blob] = ...
            ori_get_minibatch(conf, image_roidb_train(sub_db_inds));% figure label-targets and roi-targets

        net_inputs = {im_blob, rois_blob, labels_blob, ori_blob, bbox_targets_blob, bbox_loss_weights_blob};
        ...
        if ~mod(iter_, opts.val_interval) 
            if opts.do_val
                ...
                for i = 1:length(shuffled_inds_val)
                    ...
                    [im_blob, ori_blob, rois_blob, labels_blob, bbox_targets_blob, bbox_loss_weights_blob] = ...
                       ori_get_minibatch(conf, image_roidb_val(sub_db_inds));
...
function show_state(iter, train_results, val_results)
    fprintf('\n------------------------- Iteration %d -------------------------\n', iter);
    fprintf('Training : cls error %.3g, orierror %.3g, loss (ori %.3g, reg %.3g)\n', ...
        1 - mean(train_results.accuarcy_cls.data), ...
        1 - mean(train_results.accuarcy_ori.data), ...
        mean(train_results.loss_ori.data), ...
        mean(train_results.loss_bbox.data));
    if exist('val_results', 'var') && ~isempty(val_results)
        fprintf('Testing  : cls error %.3g, orierror %.3g, loss (ori %.3g, reg %.3g)\n', ...
        1 - mean(train_results.accuarcy_cls.data), ...
        1 - mean(train_results.accuarcy_ori.data), ...
        mean(train_results.loss_ori.data), ...
        mean(train_results.loss_bbox.data));

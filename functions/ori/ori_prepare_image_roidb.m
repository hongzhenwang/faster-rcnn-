function [image_roidb, bbox_means, bbox_stds] = ori_prepare_image_roidb(conf, imdbs, roidbs, bbox_means, bbox_stds)
    ...
    image_roidb = ...
        cellfun(@(x, y) ... // @(imdbs, roidbs)
                arrayfun(@(z) ... //@([1:length(x.image_ids)])
                        ...
                        'overlap', y.rois(z).overlap,'ori_overlap', y.rois(z).ori_overlap, 'boxes', y.rois(z).boxes, 'class', y.rois(z).class, 'image', [], 'bbox_targets', []), ...

function save_model_path = ori_train(conf, imdb_train, roidb_train, varargin)
...
%% training
    ...
    while (iter_ < max_iter)
        ...
        net_inputs = {im_blob, rois_blob, ori_blob};%labels_blob, bbox_targets_blob, bbox_loss_weights_blob};
...
function show_state(iter, train_results, val_results)
    fprintf('\n------------------------- Iteration %d -------------------------\n', iter);
    fprintf('Training : error %.3g, loss (ORI %.3g, reg %.3g)\n', ...
...
%         mean(train_results.loss_bbox.data));
    if exist('val_results', 'var') && ~isempty(val_results)
        fprintf('Testing  : error %.3g, loss (ORI %.3g, reg %.3g)\n', ...
            1 - mean(val_results.accuarcy.data), ...
            mean(val_results.loss_ori.data));

function [rec,prec,ap] = VOCevaldet_plane(VOCopts,id,cls,draw)

% load test set
[gtids,t]=textread(VOCopts.testset,'%s %d');
...
%     rec=PASreadrecord(sprintf(VOCopts.annopath,gtids{i}));
    load(sprintf(VOCopts.annopath, gtids{i}));
    rec = rec;
    
    % extract objects of class
%     clsinds=strmatch(cls,{rec.objects(:).class},'exact');
    tem_whz={rec.objects(:).class};
    tem2_whz=cat(1,tem_whz{:});
    clsinds=strmatch(cls,tem2_whz,'exact');
